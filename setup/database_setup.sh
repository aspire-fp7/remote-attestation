...
if [ -v forceDrop ]; then
	...
	mysql -u root -p${MYSQL_ROOT_PASSWORD} -h $serverAddress -e "DROP DATABASE $raDbName"
	checkError $?
...
mysql -u root -p${MYSQL_ROOT_PASSWORD} -h $serverAddress -e "CREATE DATABASE $raDbName" &> /dev/null
if [ $? == 0 ]; then
	...
	do
		...
		mysql -u root -p${MYSQL_ROOT_PASSWORD} -h $serverAddress < $createUserSqlDir/$userScript
	...
	do
		...
		mysql -u root -p${MYSQL_ROOT_PASSWORD} -h $serverAddress $raDbName < $createTablesSqlDir/$tablesScript
		checkError $?
	done
else
