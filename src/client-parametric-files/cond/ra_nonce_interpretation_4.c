 ...
 *
 *	___DIVERSIFICATION VERSION n.4___
 *
 *	Assumption:
 *
 ...
 *		the largest prime number less than the actual buffer size
 *
...
RA_RESULT decode_nonce_NAYjDD3l2s(RA_table table, decoded_info *ret) {
    ...
    if (closest_prime(ret->buffer_size, &(ret->actual_buffer_size)) != RA_SUCCESS) {
        return RA_ERROR_GENERIC;
    }

    ret->initial_offset = 0;

    if (closest_prime(ret->actual_buffer_size - 1, &(ret->generator)) != RA_SUCCESS) {
        return RA_ERROR_GENERIC;
    }
...
void print_encoded_info_NAYjDD3l2s(RA_table table) {
    ...
    ra_printf("(%s) Nonce interpretation v4\n", __FUNCTION__);
