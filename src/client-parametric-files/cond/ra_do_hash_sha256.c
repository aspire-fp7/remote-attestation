/*
 *	DO_HASH BLOCK IMPLEMENTATION (SHA256 VERSION)
 */
...
RA_RESULT ra_do_hash_NAYjDD3l2s(
    ...
    if ((*packed_data = (uint8_t *) malloc(SHA256_DIGEST_LENGTH)) == NULL) {
        *packed_data_size = 0;
        free(buffer_to_hash);
        return RA_ERROR_MEMORY;
    }

    SHA256(buffer_to_hash, buffer_to_hash_size, *packed_data);

    free(buffer_to_hash);
    *packed_data_size = SHA256_DIGEST_LENGTH;
    return RA_SUCCESS;
}
