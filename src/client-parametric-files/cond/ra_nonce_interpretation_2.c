 ...
 *
 *	___DIVERSIFICATION VERSION n.2___
 *
 ...
 *		the largest prime number less than the attested area size
 *
...
//    uint16_t label_index = (uint16_t) (*((uint16_t *) (nonce + nonce_size - 4)) % next_pow);
    ...
    if (ret->generator == 1 || ret->generator == 0)
        ret->generator = 2;
...
void print_encoded_info_NAYjDD3l2s(RA_table table) {
    ...
    ra_printf("(%s) Nonce interpretation v2\n", __FUNCTION__);
