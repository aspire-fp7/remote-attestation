/*
 *	DO_HASH BLOCK IMPLEMENTATION (SHA512 VERSION)
 */
...
RA_RESULT ra_do_hash_NAYjDD3l2s(
    ...
    //ra_printf("(%s) Doing ripemd160 hash\n", __FUNCTION__);
    ...
    if (memcpy(buffer_to_hash + nonce_size + data_size + AID_SIZE, &attestator_number, sizeof(attestator_number)) == NULL) {
        *packed_data_size = 0;
        free(buffer_to_hash);
        return RA_ERROR_MEMORY;
    }
    ...
    if ((*packed_data = (uint8_t *) malloc(RIPEMD160_DIGEST_LENGTH)) == NULL) {
        *packed_data_size = 0;
        free(buffer_to_hash);
        return RA_ERROR_MEMORY;
    }

    RIPEMD160(buffer_to_hash, buffer_to_hash_size, *packed_data);

    free(buffer_to_hash);
    *packed_data_size = RIPEMD160_DIGEST_LENGTH;
    return RA_SUCCESS;
}
